...
# How many markets the geography section has to show. Below two there is
# nothing to break down -- the country count on the Campaign Snapshot already
# says it all -- so the whole section comes out of the deck.
country_distribution = analysis.tables.get("Country Distribution")

market_count = (
    0 if country_distribution is None else len(country_distribution)
)

if market_count < 2:
    print(
        "\n[GEOGRAPHY] Section dropped - "
        "fewer than 2 markets in the delivered leads.\n"
    )
